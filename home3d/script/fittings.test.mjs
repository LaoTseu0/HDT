...
import {
  ...
  FITTING_ARM_FACTOR,
  FITTING_ARM_MIN,
} from '../src/lib/fittings.js'
import { PIPE_SECTIONS } from '../src/lib/plumbing.js'
...
const run = (id, points, sectionKey = 'cuivre16') => ({
  id,
...
describe('detectFittings — tolérance & garde-fous', () => {
  ...
  it('la tolérance suit les sections (évac Ø100 attrape à ~7 cm)', () => {
    const a = run('app-1', [[0, 0, 0], [4, 0, 0]], 'evac100')
    const b = run('app-2', [[2, 0.07, 0], [2, 2, 0]], 'evac40')
  ...
  it('run seul / liste vide / chemin dégénéré → rien', () => {
    ...
    assert.deepEqual(detectFittings([run('app-1', [[0, 0, 0], [1, 0, 0]])]), [])
    assert.deepEqual(
      detectFittings([run('app-1', [[0, 0, 0]]), run('app-2', [[0, 0, 0], [1, 0, 0]])]),
      []
    )
  })
})
...
describe('detectFittings — bras (dimensions du raccord)', () => {
  it('section surdimensionnée (collier) et longueur liée au plus gros tuyau', () => {
    const main = run('app-1', [[0, 0, 0], [4, 0, 0]], 'evac100')
    const branch = run('app-2', [[2, 0, 0], [2, 2, 0]], 'evac40')
    const [f] = detectFittings([main, branch])
  ...
  it('petit cuivre : longueur de bras plancher (lisibilité)', () => {
    const a = run('app-1', [[0, 0, 0], [2, 0, 0]], 'cuivre12')
    const b = run('app-2', [[2, 0, 0], [2, 0, 2]], 'cuivre12')
    const [f] = detectFittings([a, b])
    const arm = f.arms[0]
    const len = Math.hypot(arm.b[0] - arm.a[0], arm.b[1] - arm.a[1], arm.b[2] - arm.a[2])
...
describe('fittingMesh', () => {
  it('un mini-run par bras : positions/indices concaténés, indices valides', () => {
    const main = run('app-1', [[0, 0, 0], [4, 0, 0]])
    const branch = run('app-2', [[2, 0, 0], [2, 2, 0]])
    const [f] = detectFittings([main, branch])
