...
import {
  ...
  DEFAULT_ELEC_KIND,
  isElecKind,
} from '../src/lib/elec.js'
import { referencePoints, kindNaming, deriveDims } from '../src/lib/editRegistry.js'
...
describe('ELEC_COMPONENTS (catalogue)', () => {
  it('4 composants, prise = défaut, types conformes à la regex de nommage', () => {
    assert.deepEqual(ELEC_KINDS, ['elec.outlet', 'elec.switch', 'elec.junction', 'elec.meter'])
...
describe('referencePoints (composant élec)', () => {
  it('centre + 4 coins de la face arrière (sur le mur, normal=0)', () => {
    const obj = {
      kind: 'elec.junction',
      params: { largeur_m: 0.2, hauteur_m: 0.4, profondeur_m: 0.1 },
...
describe('deriveDims (composant élec)', () => {
  it('u→largeur, v→hauteur, normal→profondeur', () => {
    const obj = {
      kind: 'elec.meter',
      params: { largeur_m: 0.4, hauteur_m: 0.6, profondeur_m: 0.18 },
      plane: { origin: [0, 0, 0], u: [1, 0, 0], v: [0, 1, 0], normal: [0, 0, 1] },
    }
    assert.deepEqual(deriveDims(obj), { largeur_m: 0.4, profondeur_m: 0.18, hauteur_m: 0.6 })
  })
})
