...
import { ensureBoundsTree, meshReferencesNear } from '../src/lib/bvh.js'
...
const vclose = (a, b, eps = 1e-6) => a.every((x, i) => Math.abs(x - b[i]) <= eps)
const hasVert = (verts, target, eps = 1e-6) => verts.some((v) => vclose(v, target, eps))
