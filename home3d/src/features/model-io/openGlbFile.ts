...
export async function openGlbFile(file: File | null | undefined): Promise<void> {
  if (!file) return
  ...
  try {
    ...
    setLoadError(
      `Lecture du fichier impossible : ${err instanceof Error ? err.message : err}`
    )
  }
}

// Modèle de démo servi depuis public/models/ (pratique en dev).
export async function loadDemoModel(): Promise<void> {
  const { requestLoad, setLoadError } = useStore.getState()
  ...
  try {
    ...
    setLoadError(
      `Modèle de démo introuvable (${url}) : ${err instanceof Error ? err.message : err}`
    )
  }
}
