...
describe('plomberie.pipe (générateur)', () => {
  ...
  const obj = appObj(PIPE_KIND, payload.params, { id: 'app-3', plane: payload.plane })
...
describe('plomberie.pipe (pente E16-02 dans le registre)', () => {
  ...
  const obj = appObj(
    PIPE_KIND,
    { ...payload.params, pente_pct: 2 },
    { id: 'app-9', plane: payload.plane }
  )
  ...
  it('le câble n’est pas affecté (pas de pente hors plomberie)', () => {
    const cable = appObj('elec.cable', {
      ...
      largeur_m: 0.02,
      hauteur_m: 0.02,
      // pente_pct hors contrat câble : robustesse à un param étranger.
      ...({ pente_pct: 5 } as object),
    })
