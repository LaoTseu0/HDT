...
// testable seul (plumbing.test.ts).
//
// Même mécanique que le câble élec (E15-03, cf. cable) : un « run » = une
// polyligne balayée avec une SECTION RECTANGULAIRE (routing, réutilisé tel
...
import type { PayloadOf, PipeFamille, PipeParams, Vec3 } from '@/types'

export const PIPE_KIND = 'plomberie.pipe' as const

/** Preset de section du catalogue plomberie. */
export interface PipeSectionPreset {
  label: string
  famille: PipeFamille
  diametre_mm: number
...
// prettier-ignore
export const PIPE_SECTIONS: Record<string, PipeSectionPreset> = {
...
export function pentePct(params: Partial<PipeParams> | null | undefined): number {
 ...
 * @returns points pentus (ou `points` tel quel si pente nulle)
 */
export function slopedPoints(params: Partial<PipeParams> | null | undefined): Vec3[] {
  const pts = params?.points ?? []
  const pente = pentePct(params)
  if (!pente || pts.length < 2) return pts
  const first = pts[0]!
  const out: Vec3[] = [[first[0], first[1], first[2]]]
  let cumul = 0
  for (let i = 1; i < pts.length; i++) {
    const p = pts[i]!
    const q = pts[i - 1]!
    cumul += Math.hypot(p[0] - q[0], p[2] - q[2])
    out.push([p[0], round(p[1] - (pente / 100) * cumul), p[2]])
  }
  return out
}

/** Longueur totale (m) d'un tuyau depuis ses params (pente comprise). */
export function pipeLength(params: Partial<PipeParams> | null | undefined): number {
 ...
 * @param sectionKey preset du catalogue (défaut = cuivre Ø16)
 * @returns payload prêt pour `createObject`, ou `null` si < 2 sommets distincts.
 */
export function pipePayloadFromPath(
  points: Vec3[],
  sectionKey: string = DEFAULT_PIPE_SECTION
): PayloadOf<'plomberie.pipe'> | null {
