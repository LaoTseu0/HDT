...
/** Vecteur d'un stick (repère « jeu » : x+ droite, y+ avant/haut). */
export interface StickVec {
  x: number
  y: number
}

/** Entrées analogiques des deux sticks. */
...
export function applyDeadzone(value: number, deadzone: number = DEADZONE): number {
...
export function clampStick(x: number, y: number): StickVec {
...
export function stickFromPointer(dx: number, dy: number, radius: number): StickVec {
...
export function gamepadInput(
  gamepads: Iterable<Gamepad | null> | null | undefined
): AnalogInput | null {
...
export const touchInput: AnalogInput = {
...
export function setStick(stick: StickVec, x: number, y: number): void {
  stick.x = x
  stick.y = y
}
