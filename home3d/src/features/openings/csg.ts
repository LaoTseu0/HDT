...
import type { AppObjectOf } from '@/types'
...
const pristineGeom = new WeakMap<THREE.Mesh, THREE.BufferGeometry>()
// meshes portant actuellement une géométrie PERCÉE (pour tout restaurer).
const cutMeshes = new Set<THREE.Mesh>()

/** Ouverture perçant un mur (fenêtre ou porte). */
export type OpeningObject = AppObjectOf<'opening.window' | 'opening.door'>
 ...
 */
export function openingCutBox(opening: OpeningObject): THREE.BoxGeometry {
  const w = Math.max(Number(opening.params.largeur_m) || 0, 0.01)
  const h = Math.max(Number(opening.params.hauteur_m) || 0, 0.01)
  const { origin, u, v, normal } = frameOfObjectPlane(opening.plane)
 ...
 */
export function cutWallGeometry(
  origGeometry: THREE.BufferGeometry,
  worldMatrix: THREE.Matrix4,
  cutBoxes: THREE.BufferGeometry[]
 ...
 */
export function isCutDegenerate(
  origGeometry: THREE.BufferGeometry,
...
export function markPristine(mesh: THREE.Mesh): THREE.BufferGeometry {
...
export function applyCut(mesh: THREE.Mesh, cutGeometry: THREE.BufferGeometry): void {
 ...
 */
export function withPristineGeometry<T>(scene: THREE.Object3D, fn: () => T): T {
  const swapped: Array<[THREE.Mesh, THREE.BufferGeometry]> = []
  scene.traverse((m) => {
