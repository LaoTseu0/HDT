...
import { openingCutBox, cutWallGeometry, isCutDegenerate } from '@/features/openings/csg'
...
function makeOpening(allege = 0.8) {
  return appObj(
    'opening.window',
    { largeur_m: 1.2, hauteur_m: 1.0, allege_m: allege },
...
function raycastHitsFront(mesh: THREE.Mesh, x: number, y: number) {
  ...
  return rc
    ...
    .some((h) => h.point.z >= -0.15 && h.point.z <= 0.15)
}
