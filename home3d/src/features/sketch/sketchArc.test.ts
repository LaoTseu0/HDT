...
import { parseVcbAngle } from '@/features/sketch/vcb'
import {
  referencePoints,
  deriveDims,
  type ReferencePoint,
...
describe('referencePoints (arc)', () => {
  it('plat : centre + début + fin + milieu = 4 points', () => {
    const obj = appObj(
      'sketch.arc',
      { rayon_m: 2, angle_debut_deg: 0, angle_balayage_deg: 90 },
  ...
  it('extrudé : base ET face haute = 8 points', () => {
    const obj = appObj(
      'sketch.arc',
      { rayon_m: 1, angle_debut_deg: 0, angle_balayage_deg: 90, hauteur_m: 3 },
      { plane: { origin: [0, 0, 0], u: [1, 0, 0], v: [0, 0, 1], normal: [0, 1, 0] } }
    )
...
describe('deriveDims (arc)', () => {
  it('emprise = bounding box de l’arc dans le plan', () => {
    ...
    const dims = deriveDims(
      appObj('sketch.arc', { rayon_m: 2, angle_debut_deg: 0, angle_balayage_deg: 180 })
    )!
