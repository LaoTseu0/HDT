...
const vclose = (
  ...
  b: readonly number[],
  eps = 1e-9
) => !!a && a.every((x, i) => close(x, b[i] ?? NaN, eps))
...
describe('pickBestSnap', () => {
  ...
  it('renvoie null si aucun candidat dans le seuil', () => {
    const cands: SnapCandidate[] = [
      { type: 'endpoint', point: [0, 0, 0], sx: 200, sy: 200 },
    ]
  ...
  it('la grille accroche en l’absence de toute autre référence', () => {
    const best = pickBestSnap(
      [{ type: 'grid', point: [1, 0, 0], sx: 104, sy: 100 }],
      cursor,
      12
    )
  ...
  it('conserve les champs d’inférence (color/lines) du candidat retenu', () => {
    ...
    assert.equal((best as SnapCandidate | null)?.sx, undefined) // coordonnées écran retirées
  })
})
