...
import type { Segment3, Vec3 } from '@/types'
...
// three-mesh-bvh augmente lui-même les types de three (computeBoundsTree /
// disposeBoundsTree / boundsTree sur BufferGeometry) — rien à déclarer ici.

let patched = false
export function patchBVH(): void {
  if (patched) return
  // Cast : three-mesh-bvh type sa fonction (GeometryBVH générique) plus finement
  // que sa propre augmentation de BufferGeometry (MeshBVH) — même objet au runtime.
...
export function ensureBoundsTree(root: THREE.Object3D | null | undefined): void {
  if (!root) return
  patchBVH()
  root.traverse((child) => {
...
/** Références de proximité d'un mesh : sommets + arêtes monde, dédupliqués. */
export interface MeshReferences {
  verts: Vec3[]
  edges: Segment3[]
}
 ...
 *
 * @param mesh    mesh porteur d'un `geometry.boundsTree`
 * @param center  point de requête (monde, [x,y,z])
 * @param radius  rayon de requête (monde)
 * @param maxTris budget de triangles collectés (borne le coût)
 * @returns null si le mesh n'a pas de boundsTree (l'appelant retombe sur le
 *          triangle survolé).
 */
export function meshReferencesNear(
  ...
  const verts = new Map<string, Vec3>() // clé → [x,y,z] monde
  const edgeKeys = new Set<string>()
  const edges: Segment3[] = []
  let count = 0
  ...
  bvh.shapecast({
    ...
    intersectsTriangle: (tri) => {
      ...
      const keys = [vKey(tri.a), vKey(tri.b), vKey(tri.c)] as [string, string, string]
      const world = tv.map((v): Vec3 => {
        _w.copy(v).applyMatrix4(mw)
        return [_w.x, _w.y, _w.z]
      }) as [Vec3, Vec3, Vec3]
      for (let i = 0; i < 3; i++) {
        const key = keys[i]!
        if (!verts.has(key)) verts.set(key, world[i]!)
      }
      for (let i = 0; i < 3; i++) {
        const a = i as 0 | 1 | 2
        const b = ((i + 1) % 3) as 0 | 1 | 2
