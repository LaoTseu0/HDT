...
import type { NamingFields, ObjectsTable } from '@/types'
...
const pad3 = (n: number) =>
  String(Math.max(1, Math.trunc(Number(n) || 1)))
    .padStart(3, '0')
    .slice(-3)
 ...
 */
export function normalizeType(type: string | null | undefined, fallback: string): string {
 ...
 * @returns nom qui passe NODE_NAME_REGEX.
 */
export function nodeName({ system, type, zone, level, index }: NamingFields): string {
 ...
 * @param objects   table des objets app
 * @param bucket    { system, zone, level }
 * @param excludeId objet à ignorer (ex. lui-même lors d'un changement de zone)
 */
export function nextIndex(
  objects: ObjectsTable,
  { system, zone, level }: Pick<NamingFields, 'system' | 'zone' | 'level'>,
