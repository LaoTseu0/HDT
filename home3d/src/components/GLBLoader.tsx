...
// Chargement d'un GLB par drag & drop (E3-01) ou file picker (E3-02), avec
// feedback de chargement et erreurs affichées dans l'UI (E3-05). L'ex-toolbar
// (recentrer, éditer, visiter…) vit dans la barre latérale (E19) ; ne restent
// ici que l'état vide et les feedbacks de chargement.
export default function GLBLoader() {
  ...
  // Drag & drop sur toute la fenêtre (E3-01), avec compteur enter/leave pour un
  // feedback visuel fiable malgré les éléments imbriqués.
  useEffect(() => {
    ...
    const onDrop = (event: DragEvent) => {
      ...
      dragDepth.current = 0
      setIsDragging(false)
      void openGlbFile(event.dataTransfer?.files?.[0])
    }
