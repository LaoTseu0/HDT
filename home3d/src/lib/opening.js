...
export const WINDOW_KIND = 'opening.window'
export const DOOR_KIND = 'opening.door'
 ...
 * @param {number[]} point point d'impact sur la face (monde)
 * @param {object} frame repère de la face { type, origin, u, v, normal, faceOf? }
 * @param {{largeur_m:number, hauteur_m:number}} [dims] gabarit (E14-07, défaut `simple`)
 * @returns payload prêt pour `createObject`.
 */
export function doorPayload(point, frame, dims = DOOR_PRESETS[DEFAULT_DOOR_PRESET]) {
  const H = dims.hauteur_m
