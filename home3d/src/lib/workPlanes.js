...
// (script/workPlanes.test.mjs) et réutilisable côté générateur comme côté tracé.
 ...
 * @returns {{ u:number[], v:number[], normal:number[] }}
 */
export function makeBasisFromNormal(normal, seed = [1, 0, 0]) {
 ...
 * @param {number[]} point  point d'impact (monde)
 * @param {number[]} normal normale monde de la face
 * @param {string} [faceOf] node name du mesh (liaison, pour Slice 1 ouvertures)
 */
export function faceFrame(point, normal, faceOf) {
  const n = normalize(normal)
 ...
 * @returns {{ origin:number[], u:number[], v:number[], normal:number[] }}
 */
export function frameOfObjectPlane(plane) {
  const g = groundFrame()
  return {
 ...
 * @param {number[]} center centre de la face (monde)
 * @param {number[]} normal normale de la face (monde, unitaire)
 * @param {number[]} rayOrigin origine du rayon souris
 * @param {number[]} rayDir direction du rayon souris (unitaire)
 * @returns {number} hauteur signée le long de la normale
 */
export function extrudeHeightFromRay(center, normal, rayOrigin, rayDir) {
  const n = normalize(normal)
